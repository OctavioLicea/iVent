...
export default function EventFrontPage() {
  ...
  const activeBgPattern = !bg_url && config?.bg_pattern
    ? BG_PATTERNS.find(p => p.id === config.bg_pattern)
    : null
  const isAnimated = activeBgPattern?.animated === true
    ...
    <div style={styles.page}>
      <div style={{
        ...
        ...(bg_url
          ...
          : activeBgPattern
            ? activeBgPattern.style
            : { background: palette.surface }),
      }}>
        {/* Fondo animado con sparks — solo cuando el patrón es animated */}
        {isAnimated && (
          <AnimatedBg
            pattern={activeBgPattern.id}
            primaryColor={palette.primary}
            accentColor={palette.accent}
          />
        )}

        {/* Contenido — z-index: 1 para aparecer sobre las sparks de AnimatedBg */}
