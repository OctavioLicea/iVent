...
export default function EventDesigner() {
  ...
  // ── PREVIEW CSS VARS ──
  // --color-* → alias canónico (BG_PATTERNS usa color-mix con estas vars, también EventFrontPage)
  // --ev-*    → alias interno del EventDesigner
  // paletteVars se aplica al ed-app (raíz) para que thumbnails también resuelvan color-mix()
  const paletteVars = {
  ...
  return (
    <div className="ed-app" style={paletteVars}>
      {/* TOPNAV */}
      ...
      <div className="ed-workspace">
              ...
              {imgTab !== 'fondo' ? (
                ...
                <>
                  ...
                  <div className="ed-bg-grid">
                    {BG_PATTERNS.map(p => (
                      <div key={p.id} className={`ed-bg-opt ${selectedBg === p.id && !images.bg ? 'selected' : ''} ${p.animated ? 'animated-bg' : ''}`} onClick={() => { setSelectedBg(p.id); setImages(prev => ({ ...prev, bg: '' })) }}>
                        <div className="ed-bg-swatch" style={p.style}>
                          {p.animated && <div className="ed-bg-shimmer" />}
                        </div>
                        {p.animated && <span className="ed-bg-badge">✦ vivo</span>}
                        <span className="ed-bg-label">{p.label.replace('✦ ', '')}</span>
                      </div>
                    ))}
                  </div>
              ...
              <table className="ed-typo-table">
                ...
                <tbody>
                  {TYPO_ROLES.map(role => {
                    ...
                    const previewColor = t.color || defaultTypoColor(role.key, palette)
                    // sampleColor: siempre legible en la tabla (fondo blanco)
                    const sampleColor = t.color || '#2A1F1A'
                    return (
                      <tr key={role.key} className="ed-typo-row" onMouseEnter={() => setHighlightRole(role.key)} onMouseLeave={() => setHighlightRole(null)}>
                        ...
                        <td>
                          <div className="ed-typo-sample" style={{ fontFamily: `'${t.font}',serif`, fontSize: Math.min(t.size, 17) + 'px', fontWeight: t.bold ? 700 : 400, color: sampleColor }}>
                            {role.sample}
                          </div>
                        </td>
                        <td>
                        ...
                        <td><input type="color" className="ed-typo-color" value={previewColor.length === 7 ? previewColor : '#2A1F1A'} onChange={e => updateTypo(role.key, 'color', e.target.value)} /></td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="ed-preview-area">
          ...
          <div className="ed-event-page" style={{ ...previewVars, ...bgStyle, position: 'relative', overflow: 'hidden' }}>
            {/* AnimatedBg — sparks sobre fondo oscuro cuando el patrón es animated */}
            {(() => {
              const pat = !images.bg && BG_PATTERNS.find(p => p.id === selectedBg)
              return pat?.animated
                ? <AnimatedBg pattern={pat.id} primaryColor={palette.primary} accentColor={palette.accent} />
                : null
            })()}
